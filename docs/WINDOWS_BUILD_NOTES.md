...
- `src/exam_grader/ui.py`:
  - คืนค่า import `QApplication` ใน `ui.py` เพื่อให้ `NewExamDialog` ดึง `app_icon = QApplication.windowIcon()` ได้ถูกต้องบน Windows (ไม่เกิด `NameError`)
- `tests/test_ui.py`:
  - ปรับ assertion ใน `test_confirmed_student_number_refreshes_students_without_answer_confirmation` ให้สอดคล้องกับพฤติกรรมหลังแก้ Issue 4 (เมื่อบันทึกเลขที่ใน Issue Table ระบบจะ finalize review ทันที สถานะจึงเป็น `พร้อม · เลขที่ X` และ `latest_review is not None`)
