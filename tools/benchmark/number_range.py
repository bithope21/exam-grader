...
def main():
        ...
        records.append(
            {
                ...
                "correct": result["candidate"] == str(number),
                "observation": result,
            }
        )
    ...
    for number, stem in (("1", "IMG_0791"), ("4", "IMG_0794")):
        path = (
            Path("docs/evidence/polish/final-offscreen-final/numbers")
            / stem
            / "number_roi_processed.png"
        )
    ...
    for number in ("14", "41"):
        ...
        for digit in number:
            ...
            x += w + 15
        result = observe(
            cv2.imencode(".png", pixels)[1].tobytes(),
            matrix,
            diagnostics_dir=out / f"composite-{number}",
        )
        records.append(
            {
                "truth": number,
                "kind": "composed-existing-handwriting",
                "candidate": result["candidate"],
                "correct": result["candidate"] == number,
                "observation": result,
            }
        )
        print("composite", number, result["candidate"], flush=True)
    report = {
        "kind": "generated developer test; not real handwriting accuracy",
        "records": records,
