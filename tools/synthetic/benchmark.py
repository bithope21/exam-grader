...
def generate(seed: int):
    ...
    for question in range(60):
        ...
        choices = (
            []
            if kind == 0
            else sorted(rng.choice(5, 2 if kind == 1 else 1, replace=False).tolist())
        )
        truth.append(
            {
                "selected": ["ABCDE"[choice] for choice in choices],
...
def main():
    ...
    for seed in (101, 202, 303):
        ...
        variants = {
            "clean": image,
            "rotate3": cv2.warpAffine(
                image, rotation, (image.shape[1], image.shape[0]), borderValue=(255, 255, 255)
            ),
            "blur": cv2.GaussianBlur(image, (5, 5), 1),
            "dark": np.clip(image.astype(float) * 0.7, 0, 255).astype(np.uint8),
        ...
        for variant, pixels in variants.items():
            ...
            try:
                ...
                exact = sum(
                    a["selected"] == b["selected"] and a["classification"] == b["classification"]
    ...
    report = {
        "kind": "synthetic_probe_only",
        "auto_accept_coverage": 0,
    ...
    print(
        json.dumps(
            [{k: v for k, v in row.items() if k not in {"truth", "sha256"}} for row in results],
            ensure_ascii=False,
        )
    )
