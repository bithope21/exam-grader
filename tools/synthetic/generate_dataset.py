...
def render_answer_key_image(key_id: str, key_answers: dict[str, str], rng: np.random.Generator) -> np.ndarray:
...
def generate_dataset(seed: int, output_dir: Path) -> dict:
    ...
    def create_student_fixture(
        ...
        score, final_flags = calculate_score_and_flags(
            answers_dict, keys[key_id], base_flags
        )
    ...
    create_student_fixture(
        "clean", "clean_01.png", "key_01", "650101",
    ...
    k2 = keys["key_02"]
    create_student_fixture(
        "clean", "clean_02.png", "key_02", "650102",
    ...
    create_student_fixture(
        "clean", "clean_03.png", "key_01", "650103", wrong_answers
    )
    ...
    create_student_fixture(
        "clean", "clean_04.png", "key_01", "650104", rand_answers
    )

    # clean_05 to clean_09: All A, B, C, D, E
    for idx, choice in enumerate(CHOICES, start=5):
        create_student_fixture(
            "clean", f"clean_{idx:02d}.png", "key_01", f"6501{idx:02d}",
    ...
    blank_one["30"] = []
    create_student_fixture(
        "clean", "clean_12.png", "key_01", "650112", blank_one
    )
    ...
    create_student_fixture(
        "degraded", "degraded_01.png", "key_01", "650201", perf_ans,
        mark_type="faint_mark", base_flags=["faint_mark"]
    )
    create_student_fixture(
        "degraded", "degraded_02.png", "key_01", "650202", perf_ans,
        mark_type="faint_mark", base_flags=["faint_mark"]
    )

    # degraded_03: Heavy marks
    create_student_fixture(
        "degraded", "degraded_03.png", "key_01", "650203", perf_ans,
        mark_type="heavy_mark", base_flags=["heavy_mark"]
    )

    # degraded_04: Off-center marks
    create_student_fixture(
        "degraded", "degraded_04.png", "key_01", "650204", perf_ans,
        mark_type="off_center_mark", base_flags=["off_center_mark"]
    )
    ...
    for idx, angle in enumerate(rotations, start=5):
        create_student_fixture(
            "degraded", f"degraded_{idx:02d}.png", "key_01", f"6502{idx:02d}", perf_ans,
            transforms_list=[f"rotation:{angle}"], base_flags=["rotation"]
        )

    # degraded_11: High brightness / washed out
    create_student_fixture(
        "degraded", "degraded_11.png", "key_01", "650211", perf_ans,
        transforms_list=["brightness:1.25"], base_flags=["lighting_variation"]
    )

    # degraded_12: Low brightness / dark ambient light
    create_student_fixture(
        "degraded", "degraded_12.png", "key_01", "650212", perf_ans,
        transforms_list=["brightness:0.75"], base_flags=["lighting_variation"]
    )

    # degraded_13: Low contrast paper scan
    create_student_fixture(
        "degraded", "degraded_13.png", "key_01", "650213", perf_ans,
        transforms_list=["contrast:0.6"], base_flags=["low_contrast"]
    )

    # degraded_14: Gradient shadow across paper
    create_student_fixture(
        "degraded", "degraded_14.png", "key_01", "650214", perf_ans,
        transforms_list=["shadow"], base_flags=["shadow"]
    )

    # degraded_15: Gaussian blur
    create_student_fixture(
        "degraded", "degraded_15.png", "key_01", "650215", perf_ans,
        transforms_list=["gaussian_blur:5"], base_flags=["image_blur"]
    )

    # degraded_16: Motion blur
    create_student_fixture(
        "degraded", "degraded_16.png", "key_01", "650216", perf_ans,
        transforms_list=["motion_blur"], base_flags=["image_blur"]
    )

    # degraded_17: Medium JPEG compression (quality 60)
    create_student_fixture(
        "degraded", "degraded_17.png", "key_01", "650217", perf_ans,
        transforms_list=["jpeg_quality:60"], base_flags=["jpeg_compression"]
    )

    # degraded_18: Heavy JPEG compression (quality 35)
    create_student_fixture(
        "degraded", "degraded_18.png", "key_01", "650218", perf_ans,
        transforms_list=["jpeg_quality:35"], base_flags=["jpeg_compression"]
    )

    # degraded_19: Rotation 3° + JPEG compression 50
    create_student_fixture(
        "degraded", "degraded_19.png", "key_01", "650219", perf_ans,
        transforms_list=["rotation:3.0", "jpeg_quality:50"], base_flags=["rotation", "jpeg_compression"]
    )

    # degraded_20: Combined faint marks + shadow + rotation -1°
    create_student_fixture(
        "degraded", "degraded_20.png", "key_01", "650220", perf_ans,
        mark_type="faint_mark", transforms_list=["shadow", "rotation:-1.0"],
    ...
    double_ans["15"] = ["A", "B"]
    create_student_fixture(
        "edge_cases", "edge_case_01.png", "key_01", "650301", double_ans,
    ...
    multi_double["40"] = ["C", "E"]
    create_student_fixture(
        "edge_cases", "edge_case_02.png", "key_01", "650302", multi_double,
        base_flags=["multiple_mark"]
    )

    # edge_case_03: Check mark (✓)
    create_student_fixture(
        "edge_cases", "edge_case_03.png", "key_01", "650303", perf_ans,
        mark_type="check_mark", base_flags=["non_standard_mark"]
    )

    # edge_case_04: Slash mark (/)
    create_student_fixture(
        "edge_cases", "edge_case_04.png", "key_01", "650304", perf_ans,
        mark_type="slash_mark", base_flags=["non_standard_mark"]
    )

    # edge_case_05: Scribble / corrected answer (Q20 scribbled A, marked B)
    create_student_fixture(
        "edge_cases", "edge_case_05.png", "key_01", "650305", perf_ans,
        mark_type="scribble", base_flags=["scribble_mark"]
    )

    # edge_case_06: Solid filled bubbles (edge case style)
    create_student_fixture(
        "edge_cases", "edge_case_06.png", "key_01", "650306", perf_ans,
        mark_type="solid_bubble", base_flags=["bubble_fill_style"]
    )

    # edge_case_07 & 08: Rotation ±10°
    create_student_fixture(
        "edge_cases", "edge_case_07.png", "key_01", "650307", perf_ans,
        transforms_list=["rotation:10.0"], base_flags=["large_rotation"]
    )
    create_student_fixture(
        "edge_cases", "edge_case_08.png", "key_01", "650308", perf_ans,
        transforms_list=["rotation:-10.0"], base_flags=["large_rotation"]
    )

    # edge_case_09 & 10: Perspective skew
    create_student_fixture(
        "edge_cases", "edge_case_09.png", "key_01", "650309", perf_ans,
        transforms_list=["perspective_skew"], base_flags=["perspective_skew"]
    )
    create_student_fixture(
        "edge_cases", "edge_case_10.png", "key_01", "650310", perf_ans,
        transforms_list=["perspective_skew"], base_flags=["perspective_skew"]
    )

    # edge_case_11 & 12: Mild crop / partial sheet
    create_student_fixture(
        "edge_cases", "edge_case_11.png", "key_01", "650311", perf_ans,
        transforms_list=["crop:0.04"], base_flags=["cropped_sheet"]
    )
    create_student_fixture(
        "edge_cases", "edge_case_12.png", "key_01", "650312", perf_ans,
        transforms_list=["crop:0.08"], base_flags=["cropped_sheet"]
    )

    # edge_case_13 & 14: Duplicate student number (matching 650115 from clean)
    create_student_fixture(
        "edge_cases", "edge_case_13.png", "key_01", "650115", perf_ans,
        base_flags=["duplicate_student_no"]
    )
    create_student_fixture(
        "edge_cases", "edge_case_14.png", "key_01", "650115", perf_ans,
        base_flags=["duplicate_student_no"]
    )

    # edge_case_15: Missing student number
    create_student_fixture(
        "edge_cases", "edge_case_15.png", "key_01", None, perf_ans,
        student_no_mode="missing", base_flags=["missing_student_no"]
    )

    # edge_case_16: Unreadable student number
    create_student_fixture(
        "edge_cases", "edge_case_16.png", "key_01", "UNCERTAIN", perf_ans,
        student_no_mode="unreadable", base_flags=["unreadable_student_no"]
    )
    ...
    for q_b in range(51, 61):
        ten_blanks[str(q_b)] = []
    create_student_fixture(
        "edge_cases", "edge_case_17.png", "key_01", "650317", ten_blanks,
        base_flags=["blank_answer"]
    )

    # edge_case_18: All 60 questions blank (empty sheet)
    create_student_fixture(
        "edge_cases", "edge_case_18.png", "key_01", "650318",
        {str(q): [] for q in range(1, 61)}, base_flags=["blank_answer"]
    )

    # edge_case_19: Severe perspective skew + heavy JPEG
    create_student_fixture(
        "edge_cases", "edge_case_19.png", "key_01", "650319", perf_ans,
    ...
    combo_ans["5"] = ["A", "C"]
    create_student_fixture(
        "edge_cases", "edge_case_20.png", "key_01", "650320", combo_ans,
        mark_type="faint_mark", transforms_list=["rotation:10.0", "crop:0.04"],
...
def main():
    parser = argparse.ArgumentParser(description="Generate Synthetic OMR Answer Sheet Fixture Dataset")
