...
def main():
    ...
    paths = [Path("docs/evidence/automation/teacher-run.json"), Path("docs/evidence/vol2/teacher-results.json")]
    reports = []
    for volume, truth_path in enumerate(paths, 1):
        ...
        exam = app.exams.create(ExamDetails(f"vol.{volume} regression", "2569", "ม.1", "6", "math", count))
        ...
        for state in service.states(exam.id):
            ...
            errors = [(q + 1, a, b) for q, (a, b) in enumerate(zip(detected, expected["answers"]))
            ...
            automatic.append({"file": state["source"]["original_name"], "number": state["number"], "resolved": sum(a is not None for a in detected), "uncertain": [q+1 for q,a in enumerate(detected) if a is None]})
        before = service.issues(exam.id)
        assert not any(i["kind"] == "attendance" for i in before), "Sparse files must not invent a roster"
        ...
        while window.issue_rows:
            ...
            editor = window.issue_table.cellWidget(0, 3)
            ...
            window.issue_table.cellWidget(0, 4).click()
            settle()
        result = flow.snapshot(exam.id)
        expected_scores = []
        for expected in truth["results"]:
            source = next(s for s in service.states(exam.id)
            ...
            expected_scores.append(sum(score_answer(answer, key_answer)
                                       for answer, key_answer in zip(expected_answers, truth["key"]["answers"], strict=True)))
        ...
        assert hashes == {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in root.iterdir() if p.is_file()}
        reports.append({"volume": volume, "key_adjudications": key_edits, "automatic": automatic, "review_actions": len(before), "skipped_missing": missing, "scores": [r["score"] for r in result["results"]], "export": str(output), "source_hashes_unchanged": True})
        window.close()
    (out / "report.json").write_text(json.dumps({"platform": qt.platformName(), "volumes": reports}, ensure_ascii=False, indent=2))
