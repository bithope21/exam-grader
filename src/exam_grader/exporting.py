...
def human_readable_name(exam: dict) -> str:
    raw = "_".join(str(exam.get(field, "")).strip() for field in ("academic_year", "grade", "room", "subject", "name"))
...
def checked_filename(number: str | None, used: set[str]) -> str:
    ...
    known = normalized.isascii() and normalized.isdigit() and len(normalized) <= 6 and int(normalized) > 0
...
def checked_image(data: bytes, result: dict, key: dict) -> bytes:
    original = decode(data)
    detection = result.get("detection") or {}
    registration = detection.get("registration")
    if registration and registration.get("matrix"):
        geometry = template()
        canonical = np.zeros((geometry["height"], geometry["width"], 4), np.uint8)
        for index, (answer, correct) in enumerate(zip(result["answers"], key["answers"], strict=True), start=1):
            if index > result["max"]:
                break
            choice = answer if answer in "ABCDE" else None
            if choice is not None:
                choice_index = "ABCDE".index(choice)
                x, y, w, h = cell_rect(index, choice_index)
                color = CHECKED_COLORS["correct"] if score_answer(answer, correct) else CHECKED_COLORS["incorrect"]
                ...
                x, y, w, h = cell_rect(index, 0)
                color = CHECKED_COLORS[answer]
                if answer == "boundary_cross":
                    observation = (detection.get("answers") or [])[index - 1] if index <= len(detection.get("answers") or []) else {}
                    selected = observation.get("selected") or []
                    if len(selected) >= 2 and all(value in "ABCDE" for value in selected):
                        first = cell_rect(index, "ABCDE".index(selected[0]))
                        last = cell_rect(index, "ABCDE".index(selected[-1]))
                ...
                if answer == "multiple":
                    cv2.putText(canonical, "!", (x + w // 2 - 4, y + h - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        sx1, sy1, sx2, sy2 = geometry["score_roi"]
        ...
        cv2.putText(canonical, label, origin, font, scale, CHECKED_COLORS["score"], 2, cv2.LINE_AA)
        ...
        if output_scale < 1.0:
            original = cv2.resize(original, None, fx=output_scale, fy=output_scale, interpolation=cv2.INTER_AREA)
            scale_matrix = np.array([[output_scale, 0, 0], [0, output_scale, 0], [0, 0, 1]], dtype=np.float64)
            inverse = scale_matrix @ inverse
        output_size = (original.shape[1], original.shape[0])
        overlay = cv2.warpPerspective(canonical[:, :, :3], inverse, output_size, flags=cv2.INTER_NEAREST)
        alpha = cv2.warpPerspective(canonical[:, :, 3], inverse, output_size, flags=cv2.INTER_NEAREST)
        mask = alpha > 0
        if np.any(mask):
            opacity = alpha[mask].astype(np.float32)[:, None] / 255.0
            original[mask] = (original[mask].astype(np.float32) * (1.0 - opacity) + overlay[mask].astype(np.float32) * opacity).astype(np.uint8)
        # Checked copies are review evidence; use a quality-controlled JPEG
        # so a 14 MB PNG does not dominate exports.
        ok, encoded = cv2.imencode(".jpg", original, [cv2.IMWRITE_JPEG_QUALITY, 88, cv2.IMWRITE_JPEG_OPTIMIZE, 1])
        if not ok:
    ...
    for index, (answer, correct) in enumerate(zip(result["answers"], key["answers"], strict=True)):
        ...
        if answer in {"blank", "multiple", "boundary_cross"}:
            status = {"blank": "blank", "multiple": "multiple", "boundary_cross": "boundary"}[answer]
    ...
    ok, encoded = cv2.imencode(".jpg", canvas, [cv2.IMWRITE_JPEG_QUALITY, 88, cv2.IMWRITE_JPEG_OPTIMIZE, 1])
    if not ok:
        raise OSError("ไม่สามารถสร้างภาพผลตรวจได้")
    return encoded.tobytes()
...
def export_results(flow: Workflow, exam_id: str, output_root: Path | None = None) -> Path:
    ...
    try:
        ...
        for result in snapshot["results"]:
            ...
            (staging / relative).write_bytes(checked_image(data, result, snapshot["key"]))
            ...
            result["checked_provenance"] = {
                ...
                "color_legend": {"correct": "green", "incorrect": "red", "blank": "orange", "multiple": "purple", "boundary_cross": "yellow"},
                "template_id": (result.get("detection") or {}).get("template_id"),
        ...
        for attendance in snapshot.get("attendance", []):
            if attendance["status"] in {"absent", "excused"}:
                sheet.append([str(attendance["student_number"]), None, len(snapshot["key"]["answers"]),
                              "ขาดสอบ" if attendance["status"] == "absent" else "ลา / ได้รับยกเว้น", None])
        ...
        (staging / "_system" / "manifest.json").write_text(json.dumps(hashes, indent=2), encoding="utf-8")
